...
_Значение по умолчанию:_ **false**.\
#### 19) ui-service-path
URL-адрес статики конкретного сервиса. При загрузке статических UI-ресурсов
данный URL добавляется к доменному имени сайта и считается корнем приложения,
исходя из него формируется полноценный запрос за статическим ресурсом.
_Принимаемые значения:_ **URL в виде строки. Пример: "/auth/"**.\
_Значение по умолчанию:_ **Корень приложения - "/""**.\
#### 20) checkConfig
