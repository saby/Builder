...
async function runCompilerAndCheckForErrors(
   ...
   if (await fs.pathExists(path.join(process.cwd(), '../saby-typescript'))) {
      ...
      nodeModulesDirectory = path.join(process.cwd(), '../');
   } else {
      sabyTypescriptDirectory = path.join(process.cwd(), 'node_modules/saby-typescript');
      nodeModulesDirectory = path.join(process.cwd(), 'node_modules');
   }
    ...
    */
   await fs.ensureSymlink(nodeModulesDirectory, path.join(sourceDirectory, 'node_modules'));
...
async function clearWorkspaceFromTsConfig(sourceDirectory, tsconfig) {
   if (tsconfig) {
 ...
 */
async function strictTypescriptCompiler(taskParameters, currentTsConfig) {

}

function typescriptCompiler(taskParameters, output) {
   return async function typescriptCompiler1() {
