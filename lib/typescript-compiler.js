...
async function runCompilerAndCheckForErrors(
   ...
   try {
      ...
      if (result.length === 0) {
         logger.error({
            message: 'typescript execution error is occurred. Something has happened during tsc compilation',
            error
         });
         result.push('error TS - critical error occurred during the build');
      }
   }
...
function typescriptCompiler(taskParameters, output) {
   return async function checkTypescriptForProject() {
      ...
      const COMMON_TSC_FLAGS = '--noEmit --incremental --tsBuildInfoFile "../typescript-cache"';
