      ...
      currentRootNode.nodes.forEach((currentNode) => {
         if (currentNode.prop && currentNode.type === 'decl') {
            if (!nodePropsNames.hasOwnProperty(currentNode.prop)) {
               ...
               nodePropsNames[currentNode.prop] = {
                  index: formattedNodes.length - 1,
                  content: currentNode
               };
            } else {
               const currentPropertyLocation = nodePropsNames[currentNode.prop].index;
...
async function getCompiledLess(lessContent, filePath, pathsForImport, imports, postCssOptions) {
   ...
   if (isThemeLess) {
      ...
      result = rebaseUrls({
         ...
         sourceFile: filePath,
         css: result,
         relativePackagePath: 'themes/themeName.css'
      });
   }
