...
async function getCompiledLess(lessContent, filePath, pathsForImport, imports, postCssOptions) {
   ...
   if (isThemeLess) {
      ...
      // save relative urls in auth application. For now we can't distinguish
      // virtual services of auth from each other on a server-side requirejs.
      if (urlServicePath === '/auth/') {
         rebaseOptions.relativePackagePath = 'themes/themeName.css';
         delete rebaseOptions.resourcesUrl;
      }
      ...
      // destination
      result = rebaseUrls(rebaseOptions);
   }
