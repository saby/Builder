...
async function dependencyWalker(
   ...
   if (!currentDepTree.has(currentDependency)) {
      ...
      if (
         ...
         // add a sign of self library private dependency for further
         // processing of its names.
         if (newDepTree.size === 1) {
            dependencyContent.selfLibraryDependency = true;
         }
      ...
      enter(node) {
         if (node.type === 'CallExpression' && node.callee.type === 'Identifier' && node.callee.name === 'define') {
            ...
            privateDependenciesOrder
               .filter(dep => dep.selfLibraryDependency)
               .forEach((dep) => {
                  ...
                  if (dep.moduleName.startsWith('UI')) {
                     libraryDebugLogs.push(`Indexes of ${dep.moduleName} in dependencies list: ${privateDepIndexes}`);
                  }
                  privateDependencyIndex = libDependenciesList.indexOf(dep.moduleName);
                  if (libraryDependenciesMeta[dep.moduleName].names[0]) {
                     if (dep.moduleName.startsWith('UI')) {
                        libraryDebugLogs.push(`${dep.moduleName} has a variable name in libraryDependenciesMeta: ${libraryDependenciesMeta[dep.moduleName].names[0]}`);
                     }
                     ...
                     if (dep.moduleName.startsWith('UI')) {
                        if (results.length === 0) {
                           libraryDebugLogs.push(`${dep.moduleName} variable usage wasn't found!!!`);
                        } else {
                           libraryDebugLogs.push(`${dep.moduleName} variable usage was found here:`);
                           results.forEach((currentResult) => {
                              libraryDebugLogs.push(`index: ${currentResult.index} astPath: ${currentResult.path}`);
                              libraryDebugLogs.push(escodegen.generate(functionCallbackBody.body[currentResult.index]));
                           });
                        }
                        libraryDebugLogs.push(escodegen.generate(ast));
                     }
                     results.forEach((currentResult) => {
                        ...
                        libraryDebugLogs.push(`processed expression : ${escodegen.generate(functionCallbackBody.body[currentResult.index])}`);
                     });
                  }
               });
