...
const LAZY_DEFINE_FUNCTION = 'function lazyDefineProperty(scope, name, moduleName, factory) {\n' +
...
const LAZY_DEFINE_CALL = "lazyDefineProperty(exports, 'default', 'moduleName', function() {return factory;});";
const
   ...
   pMap = require('p-map'),
   lazyDefineCallAst = esprima.parse(LAZY_DEFINE_CALL).body[0],
   definePropertyFunction = esprima.parse(LAZY_DEFINE_FUNCTION),
   ...
   traverse(ast, {
      enter(node) {
         if (node.type === 'CallExpression' && node.callee.type === 'Identifier' && node.callee.name === 'define') {
            ...
            privateDependenciesOrder.forEach((dep) => {
               ...
               while (privateDependencyIndex !== -1) {
                  ...
                  statementIndexes.forEach((currentIndex) => {
                     ...
                     const resultExportCode = { ...lazyDefineCallAst };
                     ...
                     // this code below changes this code: "function() {return <current variable name>;}"
                     resultExportCode.expression.arguments[3].body.body[0].argument =
                        functionCallbackBody.body[currentIndex].expression.right;

                     // this code transmits moduleName into lazyDefineProperty function
                     // <libraryName>:<name of exported option>
                     resultExportCode.expression.arguments[2] = {
                        type: 'Literal',
            ...
            // insert a function that defines current export property only once
            functionCallbackBody.body.unshift(definePropertyFunction);
         }
      }
   });
   const libraryResult = {
