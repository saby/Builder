         ...
         if (node.type === 'CallExpression' && node.callee.type === 'Identifier' && node.callee.name === 'define') {
            ...
            privateDependenciesOrder
               ...
               .forEach((dep) => {
                  ...
                  if (dep.moduleName === 'UI/_builder/Config') {
                     libraryDebugLogs.push(`Indexes of UI/_builder/Config in dependencies list: ${privateDepIndexes}`);
                  }
                  privateDependencyIndex = libDependenciesList.indexOf(dep.moduleName);
                  if (libraryDependenciesMeta[dep.moduleName].names[0]) {
                     if (dep.moduleName === 'UI/_builder/Config') {
                        libraryDebugLogs.push(`UI/_builder/Config has a variable name in libraryDependenciesMeta: ${libraryDependenciesMeta[dep.moduleName].names[0]}`);
                     }
                     ...
                     if (dep.moduleName === 'UI/_builder/Config') {
                        if (results.length === 0) {
                           libraryDebugLogs.push('UI/_builder/Config variable usage wasn\'t found!!!');
                        } else {
                           libraryDebugLogs.push('UI/_builder/Config variable usage was found here:');
