      ...
      traverse(currentElement, {
         enter(node) {
            if (node.name === variableName || (variableNamesFromCallback.includes(node.name))) {
                ...
                * 1) .id - var myVar = ...
                * 2) .label - { myVar: ... }
                * 3) .left.property - exports.myVar = ...
                * 4) .key -  ... { myVar: ... } ...
                */
               if (!(pathToNode.endsWith('.id') || pathToNode.endsWith('.label') || pathToNode.endsWith('.left.property') || pathToNode.endsWith('.key'))) {
                  results.push({
                     index,
                     path: this.path()
                  });
                  ...
                  results.forEach((currentResult) => {
                     ...
                     while (currentPath.length !== 0) {
                        ...
                        if (currentPath.length === 0) {
                           currentNode[currentProperty].name = `${dep.moduleName.replace(invalidCharsForVariable, '_')}_func()`;
                        } else {
            ...
            privateDependenciesOrder.forEach((dep) => {
               ...
               while (privateDependencyIndex !== -1) {
                  ...
                  statementIndexes.forEach((currentIndex) => {
                     ...
                     resultExportCode.expression.arguments[2].properties[0].value.body.body[0].argument =
