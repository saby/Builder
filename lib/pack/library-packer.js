            ...
            privateDependenciesOrder
               ...
               .forEach((dep) => {
                  ...
                  if (dep.moduleName === 'UI/_builder/Config') {
                     logger.debug(`Indexes of UI/_builder/Config in dependencies list: ${privateDepIndexes}`);
                  }
                  privateDependencyIndex = libDependenciesList.indexOf(dep.moduleName);
                  if (libraryDependenciesMeta[dep.moduleName].names[0]) {
                     if (dep.moduleName === 'UI/_builder/Config') {
                        logger.debug(`UI/_builder/Config has a variable name in libraryDependenciesMeta: ${libraryDependenciesMeta[dep.moduleName].names[0]}`);
                     }
                     ...
                     if (dep.moduleName === 'UI/_builder/Config') {
                        if (results.length === 0) {
                           logger.debug('UI/_builder/Config variable usage wasn\'t found!!!');
                        } else {
                           logger.debug('UI/_builder/Config variable usage was found here:');
                           results.forEach((currentResult) => {
                              logger.debug(`index: ${currentResult.index} astPath: ${currentResult.path}`);
                              logger.debug(escodegen.generate(functionCallbackBody.body[currentResult.index]));
                           });
                        }
                        logger.debug(escodegen.generate(ast));
                     }
                     results.forEach((currentResult) => {
