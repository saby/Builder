...
function getExternalDependencies(internalModules, dependencies) {
   ...
   dependencies.forEach((currentDependency) => {
      let normalizedDependencyName = currentDependency.startsWith('css!') ? currentDependency : currentDependency.split(/\?|!/).pop();
      ...
      if (isExternalDependency) {
         ...
         // to require "exports" as common external dependency
            ...
            externalDependencies.argumentSubstitutions.set(
               currentDependency,
               currentDependency.replace(invalidCharsForVariable, '_')
            );
         }
      }
      ...
      externalDependencies.argumentSubstitutions.set(
         currentDependency,
         isExternalDependency ? currentDependency.replace(invalidCharsForVariable, '_') : normalizedDependencyName.replace(invalidCharsForVariable, '_')
      );
   });
 ...
 * @returns {[]}
 */
function convertModulesToBeLazy(modulesContent) {
               ...
               node.arguments.forEach((argument, index) => {
                  switch (argument.type) {
                     ...
                     case 'Literal':
                        ...
                        if (index === 0) {
                           internalModules.set(argument.value, argument.value.replace(invalidCharsForVariable, '_'));
   ...
   resultCode.push(`require(["${externalDependencies.common.join('","')}"],function(${externalDependencies.common.map(element => externalDependencies.argumentSubstitutions.get(element)).join(',')}) {`);
   ...
   parsedModules.forEach((currentParsedModule) => {
      ...
      const argumentsList = currentParsedModule.dependencies
         .filter((module, index) => index < currentParsedModule.numberOfArguments)
         .map((currentModule) => {
      ...
      defineBlock.push(
         `define('${currentParsedModule.moduleName}', ${currentParsedModule.cssDependencies.length > 0 ? `['${currentParsedModule.cssDependencies.join("','")}'], ` : ''} function() {` +
...
async function writeCustomPackage(
   ...
   if (packageConfig.optimized) {
      const lazyResult = convertModulesToBeLazy(result);
      result = lazyResult;
   }
   await fs.outputFile(
