...
async function generateCustomPackage(
   ...
   const
      ...
      rootForCache = taskParameters.config.cachePath && taskParameters.config.distributive ? `${taskParameters.config.cachePath}/incremental_build` : applicationRoot,
   ...
   if (cssModulesFromOrderQueue.css.length > 0) {
      ...
      const cssRes = await customPackCSS(
         ...
         taskParameters.config.cachePath && taskParameters.config.distributive ? `${taskParameters.config.cachePath}/incremental_build` : root,
