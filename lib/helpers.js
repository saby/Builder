...
function visualizeErrorInCodeWithLine(text, lineWithError) {
   ...
   const result = [];
   for (let currentLine = lineWithError - 10; currentLine <= lineWithError.lineNumber + 10; currentLine++) {
...
function getRelativePath(root, filePath, outputRoot) {
   let pathWithoutRoot = unixifyPath(filePath).replace(root, '');
   if (outputRoot) {
