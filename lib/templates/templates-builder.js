...
class TemplatesBuilder {
   ...
   requireView() {
      ...
      if (!this.ViewBuilderTmpl) {
         this.ViewBuilderTmpl = global.requirejs('UI/Builder').Tmpl;
      }
      if (!this.ViewConfig) {
         this.ViewConfig = global.requirejs('UI/Builder').Config;
      }
      if (!this.Compiler) {
         this.Compiler = global.requirejs('UI/Builder').Compiler;
      }
   }

   async render(_options) {
      const html = await new Promise((resolve, reject) => {
         global.requirejs(['UI/Base', 'Application/State', 'UI/State', 'UI/Utils'], (UIBase, AppState, UIState, UIUtils) => {
            const config = {
            ...
            try {
               this.appInit.startRequest(config, new AppState.StateReceiver(UIState.Serializer, UIUtils.Logger));
