 ...
 */
function getTranspileOptions(relativePath, moduleName, moduleContent, extraOptions) {
   const currentCompilerOptions = { ...compilerOptions, ...extraOptions };
 ...
 * @returns {Object}
 */
function compileEsAndTs(relativePath, text, interfaceModule, extraOptions = { development: {} }) {
   const startTime = Date.now();
   const moduleName = transliterate(relativePath).replace(/\.(es|ts|tsx)$/, '');
   const result = {};
   Object.keys(extraOptions).forEach((currentBuildMode) => {
      let buildResult;
      const transpileOptions = getTranspileOptions(relativePath, moduleName, text, extraOptions[currentBuildMode]);
      try {
         buildResult = typescript.transpileModule(text, transpileOptions);
      } catch (error) {
         throw new Error(`An error occurred while compiling js from ts source: ${error.message}\n${error.stack}`);
      }
      try {
         result[currentBuildMode] = normalizeDependencies(
            buildResult.outputText,
            moduleName,
            interfaceModule,
            startTime
         );
      } catch (error) {
         const message = [`An error occurred while parsing compiled js file: ${error.message}`];
         if (error.lineNumber) {
            message.push(helpers.visualizeErrorInCodeWithLine(buildResult.outputText, error.lineNumber));
         }
