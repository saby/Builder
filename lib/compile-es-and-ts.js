...
function getTranspileOptions(relativePath, moduleName, moduleContent) {
   const currentCompilerOptions = relativePath.endsWith('.tsx') ? { ...compilerOptions, jsx: 'react-jsx' } : { ...compilerOptions };
...
function compileEsAndTs(relativePath, text, interfaceModule) {
   ...
   let result;
   try {
      result = typescript.transpileModule(text, transpileOptions);
   } catch (error) {
   ...
   try {
      ...
      if (error.lineNumber) {
         message.push(helpers.visualizeErrorInCodeWithLine(result.outputText, error.lineNumber));
      }
